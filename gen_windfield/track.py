...
from maputils import bearing2theta
...
TCRM_FMTS = ('i', datetime, 'f', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8')

TCRM_CNVT = {
    0: lambda s: int(float(s.strip() or 0)),
    1: lambda s: datetime.strptime(s.decode("ascii").strip(), ISO_FORMAT),
...
def readTrackData(trackfile):
    ...
    #try:
    return np.loadtxt(trackfile, skiprows=1,
                          comments='%',
                          delimiter=',',
                          dtype={
                          'names': TCRM_COLS,
